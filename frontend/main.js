async function askQuestion() {
    ...
    if (data.related_questions && data.related_questions.length) {
        ...
        data.related_questions.forEach(q => {
            chatbox.innerHTML += `<p class="related">- ${escapeHtml(q)}</p>`;
        });
    }

    chatbox.innerHTML += `<p><button class="feedback-btn" onclick="sendFeedback('${escapeJs(question)}', '${escapeJs(data.answer||'')}')">👍 Gửi Feedback</button></p>`;
